...
$(function () {
  ...
  $(document).on('change', 'input[type="file"][multiple]', function () {
    ...
    if (typeof maxFiles !== "undefined") {
      if (this.files.length > maxFiles) {
        if (typeof reachedAllowedFilesLimit !== "undefined") {
          showResponseMessage(form, reachedAllowedFilesLimit + ' ' + maxFiles, false);
        }
  ...
  $(document).on('show.bs.modal', '.modal', function () {
    // Calculates z-index based on the number of visible modals.
    // 1040 is the base z-index value for Bootstrap modals, and
    // each additional modal gets 10 more z-index units:
  ...
  // Hide/show the quoted message (ticket reply):
  // @version 2.2
  $('.show-quoted-message').on('click', function () {
    var id = $(this).attr('id');
