 ...
 * @version 2.3
 */
function makeCheckboxRequest($sender) {
    var ajax = {};
    var id = $($sender).val();

    ajax.method = 'POST';
    ajax.url = $($sender).attr('data-url');
    ajax.data = { id: id, z_csrf: csrfToken };

    ajax.success = function () {
        var $title = $('#checkbox-title-' + id);

        if ($title.hasClass('line-through')) {
...
function resetForm($form) {
    ...
    $('[data-is-editor="true"]').each(function () {
 ...
 * @returns string
 */
function replaceUnderscoreWithDash(data) {
...
function manageSuccessResponse($form, response) {
    ...
    if (response != false) {
        ...
        else if (rs == 'ai_writer') {
            $('#ai-writer #response-ai').val(response.message);
            $('#ai-writer .output, #use-ai-response').removeClass('d-none');
        }
    }
}
 ...
 * @return string HTML
 */
function getSpinnerMarkup(isDark) {
    var $markup;

    $markup = '<div class="loadingio-spinner-rolling-icon">';
    $markup += '<div class="ldio-icon-inner' + (isDark ? ' dark' : '') + '">';
    $markup += '<div></div>';
    $markup += '</div>';
    $markup += '</div>';
...
function formAjaxRequest($form) {
    ...
    JSON = {
        ...
        beforeSend: function () {
            ...
            console.log( $form.attr('data-dark') === 'true' );
            submitButton.html(getSpinnerMarkup($form.attr('data-dark') === 'true'));
        },
...
function getRecord(id, source, $element) {
    ...
    var spinner = getSpinnerMarkup(false);
