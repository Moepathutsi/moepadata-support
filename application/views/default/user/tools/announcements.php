<?php defined('BASEPATH') or exit('No direct script access allowed'); ?>
<div class="z-posts container mt-5 <?php echo (db_config('module_tickets') == 1) ? 'extra-height-2' : 'extra-height-1 mb-5'; ?>">
    <div class="row mb-4">
        <div class="col">
            <h2 class="h4 fw-bold mb-0"><?php echo lang('announcements'); ?></h2>
        </div>
        <!-- /col -->
    </div>
    <!-- /.row -->
    <div class="row row-main">
        <div class="col">
            <?php if (! empty($announcements)) {
                foreach ($announcements as $announcement) { ?>
                    <div class="post shadow-sm p-4 rounded">
                        <div class="clearfix">
                            <p class="h5 fw-bold"><a href="<?php echo env_url('user/tools/announcement/' . html_escape($announcement->id)); ?>"><?php echo html_escape($announcement->subject); ?></a></p>
                        </div>
                        <span class="d-inline-block small mb-2">
                            <i class="far fa-clock"></i> <?php printf(lang('posted_on'), get_date_time_by_timezone(html_escape($announcement->created_at), true)); ?>
                        </span>
                        <p class="text-muted mb-0"><?php echo get_sized_text(strip_tags($announcement->announcement), 345, true); ?></p>
                        <a class="mt-2 d-inline-block" href="<?php echo env_url('user/tools/announcement/' . html_escape($announcement->id)); ?>">
                            <?php echo lang('read_more'); ?> <i class="fas fa-angle-<?php echo (lang('theme_direction') == 'rtl') ? 'left' : 'right'; ?>"></i>
                        </a>
                    </div>
                    <!-- /.post -->
                <?php }
            } else { ?>
                <div class="text-center shadow-sm p-4 rounded">
                    <img class="not-found mt-2 mb-4" src="<?php illustration_by_color('not_found'); ?>" alt="">
                    <h2 class="h4 fw-bold"><?php echo lang('no_records_found'); ?></h2>
                </div>
            <?php } ?>
        </div>
        <!-- /col -->
    </div>
    <!-- /.row -->
    <div class="row">
        <div class="col">
            <div class="clearfix pagination-wrapper"><?php echo $pagination; ?></div>
        </div>
        <!-- /.col -->
    </div>
    <!-- /.row -->
</div>
<!-- /.container -->

<?php load_view('home/still_no_luck'); ?>