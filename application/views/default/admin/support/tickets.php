      ...
      <div class="col-sm-12">
        ...
        <div class="card">
              ...
              <div class="ml-auto">
                <div class="dropdown">
                  <button class="btn btn-success text-sm" type="button" id="create-ticket-menu" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <i class="fas fa-plus mr-2"></i> <?php echo lang('create_ticket'); ?>
                  </button>
          ...
          <div class="card-body pt-0 pb-0 records-card-body">
            <form class="z-form" action="<?php echo admin_action('support/bulk_tickets_action'); ?>" method="post">
              <input type="hidden" name="area" value="<?php echo $this->uri->segment(3); ?>">
              <div class="p-3 border-bottom d-none" id="action-box">
                <div class="response-message"></div>
                <p class="text-danger small mb-1"><?php echo lang('select_carefully_note'); ?></p>
                <div class="row">
                  <div class="col">
                    <select class="form-control select2 search-disabled" name="action" data-placeholder="<?php echo lang('select_action'); ?>" required>
                      <option value=""></option>
                      <option value="close"><?php echo lang('close'); ?></option>
                  ...
                  <!-- /.col -->
                  <div class="col pl-0">
                    <button class="btn btn-primary" type="submit"><i class="fas fa-check mr-2"></i> <?php echo lang('apply'); ?></button>
                  </div>
                  <!-- /.col -->
                </div>
                <!-- /.row -->
                    ...
                    <tr>
                      <th class="th-1">
                        <div class="icheck icheck-primary d-inline-block">
                          <input type="checkbox" id="checkboxes-toggler">
                          <label for="checkboxes-toggler"></label>
                        </div>
                      </th>
                    ...
                    if (! empty($tickets)) {
                      foreach ($tickets as $ticket) { ?>
                        <tr id="record-<?php echo html_escape($ticket->id); ?>">
                          <td>
                            <div class="icheck icheck-primary d-inline-block">
                              <input class="checkbox-item" type="checkbox" id="checkbox-<?php echo intval($ticket->id); ?>" value="<?php echo intval($ticket->id); ?>" name="ids[]">
                              <label for="checkbox-<?php echo intval($ticket->id); ?>"></label>
                            </div>
                          </td>
                              ...
                              <a class="mr-1" href="<?php echo env_url('admin/tickets/ticket/' . html_escape($ticket->id)); ?>">
                                ...
                                <?php if ($ticket->feedback_type !== null) { ?>
                                  <?php if ($ticket->feedback_type == 0) { ?>
                                    <i class="far fa-thumbs-down text-danger small" title="<?php echo lang('poor_received_support'); ?>"></i>
                                  <?php } else if ($ticket->feedback_type == 1) { ?>
                                    <i class="far fa-thumbs-up text-success small" title="<?php echo lang('great_received_support'); ?>"></i>
                                  <?php } ?>
                                <?php } ?>
