 ...
 * @version 2.5
 */
if (! function_exists('is_openai_ready')) {
    function is_openai_ready()
    {
        $ci = &get_instance();
        return db_config('openai_status') == 1 && db_config('openai_api_key') != '' && $ci->zuser->has_permission('ai_writer');
    }
}
...
if (! function_exists('is_admin_panel_allowed')) {
    function is_admin_panel_allowed()
    {
        ...
        if (
            $ci->zuser->has_permission('canned_replies') ||
