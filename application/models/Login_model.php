    ...
    public function social_user_data( $user, $provider )
    {
        ...
        if ( empty( $data ) )
        {
            ...
            if ( ! empty( $user->photoURL ) )
            {
                ...
                $image_contents = file_get_contents( $user->photoURL );
                $save_path = FCPATH . IMG_UPLOADS_DIR . 'users/' . $image_name;

                if ( ! empty( $image_contents ) )
                {
                    if ( file_put_contents( $save_path, $image_contents ) )
                    {
                        $data['picture'] = $image_name;
                    }
                    else
                    {
                        $data['picture'] = do_secure_url( $user->photoURL );
                    }
                }
                else
                {
                    $data['picture'] = DEFAULT_USER_IMG;
                }
            }
            else
            {
