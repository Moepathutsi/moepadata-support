...
class Support extends MY_Controller
{
     ...
     * @version 2.4
     */
    public function bulk_chats_action()
    {
        check_action_authorization('chats');

        $action = post('action');

        if (! in_array($action, ['end', 'delete'])) {
            r_error('invalid_req');
        }

        $ids = post('ids');
        $area = do_secure_l(post('area'));

        if (! is_array($ids)) r_error('select_entries');
        ...
        if ($action == 'end') {
            foreach ($ids as $id) {
                ...
                if (! empty($chat)) {
                    $this->Support_model->end_chat($id);
                }
            }
        } else {
            foreach ($ids as $id) {
                $this->delete_chat_process($id, true);
            }
        }

        r_s_jump("admin/chats/{$area}", 'action_done');
    }
     ...
     * @version 2.4
     */
    private function delete_chat_process($id, $bulk = false)
    {
        $chat = $this->Support_model->chat($id);
     ...
     * @version 2.3
     */
    private function delete_ticket_process($id, $bulk = false)
    {
        ...
        if ($this->Support_model->delete_ticket($id)) {
            ...
            if (! empty($replies)) {
                ...
                foreach ($replies as $reply) {
                    ...
                    $this->Support_model->delete_ticket_reply_quotes($reply->id);
                }
            }
            ...
            $this->Support_model->delete_ticket_tasks($id);
            $this->Support_model->delete_ticket_envato_response($id);
            $this->Support_model->delete_logged_email_ticket_references($id);
     ...
     * @version 2.3
     */
    public function bulk_tickets_action()
    {
        check_action_authorization('tickets');

        $action = post('action');

        if (! in_array($action, ['close', 'open', 'delete'])) {
            r_error('invalid_req');
        }
        ...
        if ($action == 'close') {
            ...
            foreach ($ids as $id) {
                $this->Support_model->update_ticket_status($id, 1, '', false);

                log_ticket_activity('ticket_reopened_agent', $id);
            }
        } else {
            if (!$this->zuser->has_permission('delete_ticket')) {
                r_error('no_permission');
            }

            foreach ($ids as $id) {
                $this->delete_ticket_process($id, true);
            }
        }

        r_s_jump("admin/tickets/{$area}", 'action_done');
    }
     ...
     * @version 2.3
     */
    public function add_ticket_task()
    {
        check_action_authorization('tickets');
        ...
        if ($this->form_validation->run('support_task')) {
            $user_id = $this->zuser->get('id');

            $data = [
                'title' => do_secure(post('title')),
                'priority' => do_secure(post('priority')),
                'description' => do_secure(post('description'), true),
                'user_id' => $user_id,
                'ticket_id' => $ticket->id,
                'status' => TASK_PENDING,
                'created_at' => time()
            ...
            $id = $this->Support_model->add_ticket_task($data);

            if (! empty($id)) {
                log_ticket_activity('ticket_task_added', $ticket->id, $user_id);

                r_s_jump("admin/tickets/ticket/{$ticket->id}", 'task_added');
            }
    ...
    /**
     * Edit Ticket Task ( Response ).
     *
     * @return  void
     * @version 2.3
     */
    public function edit_ticket_task()
    {
        check_action_authorization('tickets');

        if (! post('id')) r_error('invalid_req');

        $data = $this->Support_model->ticket_task(intval(post('id')));

        if (! empty($data)) {
            display_view('admin/responses/forms/edit_ticket_task', $data);
        }
    ...
    /**
     * Update Ticket Task Input Handling
     * 
     * @return void
     * @version 2.3
     */
    public function update_ticket_task()
    {
        check_action_authorization('tickets');

        $id = intval(post('id'));

        $task = $this->Support_model->ticket_task($id);

        if (empty($task)) r_error('invalid_req');
        ...
        if ($this->form_validation->run('support_task')) {
            ...
            if ($this->Support_model->update_ticket_task($data, $id)) {
                log_ticket_activity('ticket_task_updated', $task->ticket_id, $user_id);

                r_s_jump("admin/tickets/ticket/{$task->ticket_id}", 'task_updated');
            }
    ...
    public function update_task_status()
    {
        ...
        if (!empty($task)) {
            $status = ($task->status == 1) ? TASK_PENDING : TASK_COMPLETED;

            if ($this->Support_model->update_ticket_task(['status' => $status], $id)) {
                log_ticket_activity('ticket_task_status_updated', $task->ticket_id, $this->zuser->get('id'));
            }
        }
     ...
     * @version 2.3
     */
    public function delete_ticket_task()
    {
        check_action_authorization('tickets');
        ...
        if ($this->Support_model->delete_ticket_task($id)) {
            log_ticket_activity('ticket_task_deleted', $task->ticket_id, $this->zuser->get('id'));

            r_success_remove($id);

            // r_s_jump( "admin/tickets/ticket/{$task->ticket_id}", 'task_deleted' );
        }
    ...
    public function add_reply()
    {
        ...
        if ($ticket->source == 2 && ! is_email_settings_filled()) {
            r_error('requires_email_enable');
        }
        ...
        if ($this->form_validation->run('add_reply')) {
            ...
            if (! empty($id)) {
                ...
                if (!empty($_FILES)) {
                    ...
                    $max_files = intval(db_config('max_files'));
                    $files = array_slice($_FILES, 0, $max_files);
                    ...
                    foreach ($files as $key => $file) {
                        if (!empty($file['tmp_name'])) {
                            ...
                            $email_attachments[] = $file['full_path'];
                        }
                    }
                }
                ...
                } else if ($ticket->source == 2) {
                    // Email to ticket based notification:
                    send_agent_reply_to_email($ticket_id, $id, $data['message'], $email_attachments);
                }
        ...
        if ($this->form_validation->run($key)) {
            ...
            if (! empty($id)) {
                ...
                if (!empty($_FILES)) {
                    ...
                    $files = array_slice($_FILES, 0, intval(db_config('max_files')));
    ...
    public function delete_reply()
    {
        ...
        if ($this->Support_model->delete_ticket_reply($id)) {
            ...
            $this->Support_model->delete_ticket_reply_quotes($id);
