...
class Settings extends MY_Controller
{
     ...
     * @version 2.2
     */
    public function test_imap_settings()
    {
        if (version_compare(PHP_VERSION, '8.4', '<')) {
            r_error('incompatible_php');
        }

        if (!extension_loaded('iconv')) {
            r_error('no_iconv_library');
        }

        $this->load->library('ZImap');

        $status = $this->zimap->check_dependencies();

        if ($status !== true) r_error($status);

        $connection = $this->zimap->test_connection();

        if ($connection === true) {
            r_success('imap_tested');
        }

        d_r_error($connection);
    }

    /**
     * IMAP Settings Page Input Handling.
     *
     * @return  void
     * @version 2.2
     */
    ...
    {
        $data = [
            'imap_host' => do_secure(post('imap_host')),
            'imap_protocol' => do_secure(post('imap_protocol')),
            'imap_username' => do_secure(post('imap_username')),
            'imap_password' => do_secure(post('imap_password')),
            'imap_encryption' => do_secure(post('imap_encryption')),
            'imap_port' => intval(post('imap_port')),
            'imap_validate_cert' => only_binary(post('imap_validate_cert'))
        ];
    ...
    public function general()
    {
        if ($this->form_validation->run('settings_general')) {
            $data = [
                ...
                'site_custom_js' => post('site_custom_js'),
    ...
    public function support()
    {
        if ($this->form_validation->run('settings_support')) {
            ...
            $data = [
                ...
                'tickets_default_department_id' => intval(post('tickets_default_department_id')),
                'tickets_default_priority' => do_secure_l(post('tickets_default_priority')),
                ...
                'enable_email_to_ticket' => only_binary(post('enable_email_to_ticket')),
                'max_files' => intval(post('max_files'))
            ];

            $department = $this->Support_model->department($data['tickets_default_department_id']);

            if (empty($department)) {
                r_error('invalid_department');
            }

            if (! in_array($data['tickets_default_priority'], ['low', 'medium', 'high'])) {
    ...
    {
        if ($this->form_validation->run('settings_apis')) {
            $data = [
                ...
                'openai_status' => only_binary(post('openai_status')),
                'openai_api_key' => do_secure(post('openai_api_key')),
                'openai_model' => do_secure(post('openai_model')),
                'openai_max_tokens' => intval(post('openai_max_tokens')),
                'openai_temperature' => doubleval(post('openai_temperature')),
                'openai_frequency_penalty' => doubleval(post('openai_frequency_penalty')),
                'openai_presence_penalty' => doubleval(post('openai_presence_penalty')),
                'openai_top_p' => doubleval(post('openai_top_p'))
            ];
